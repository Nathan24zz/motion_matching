...
def human_mediapipe_detection(image_path, pose):
    ...
    if results.pose_landmarks:
        ...
        img_stick_man = np.zeros([480,640,3])
        visualize(img_stick_man, keypoints=keypoints)
        ...
        return image_points, cv2.resize(image, (320, 240)), cv2.resize(img_stick_man, (320, 240))
...
def robot_rcnn_detection(image_path, model):
    ...
    img_stick_man = np.zeros([480,640,3])
    visualize(img_stick_man, bboxes, keypoints)
    ...
    image_points.append(input_new_coords)
    return image_points, cv2.resize(orig_frame, (320, 240)), cv2.resize(img_stick_man, (320, 240))
    ...
    if keypoints is not None:
        for kps in keypoints:
            ...
            for idx, kp in enumerate(kps):
                ...
                image = cv2.circle(image, kp, 4, (0, 0, 255), -1)
